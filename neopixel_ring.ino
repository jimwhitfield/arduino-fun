...
void loop() {
  rainbowPartialCycle(10,2,false,0,4);
  rainbowPartialCycle(10,2,false,0,3);
  rainbowPartialCycle(10,2,false,0,2);
...
void rainbowPartialCycle(uint8_t wait, int loopCount, boolean usePastel, float base, float portion) {
  ...
  for(j=0; j<256*loopCount; j++) { // loopCount cycles of  colors on wheel
    for(i=0; i< strip.numPixels(); i++) {
      if (usePastel) {
